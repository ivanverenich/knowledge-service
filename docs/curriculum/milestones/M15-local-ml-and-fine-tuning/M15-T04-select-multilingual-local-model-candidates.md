---
id: M15-T04
title: "Select multilingual local model candidates"
milestone: M15
track: optional
depends_on: ["M15-T03"]
estimate: 75m
topics: ["ollama","vllm","qlora"]
deliverables: ["local model selection matrix"]
status: planned
---

# M15-T04 — Select multilingual local model candidates

## Outcome

Use licenses, Ukrainian/English evidence, context/tool/structure support, hardware fit, and evaluation cost to shortlist models rather than chasing leaderboards.
...
2. Use licenses, Ukrainian/English evidence, context/tool/structure support, hardware fit, and evaluation cost to shortlist models rather than chasing leaderboards.
...
- A selection matrix identifies one candidate to test and clear rejection reasons.
...
**Implementation prompt:** Inspect M15-T04, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Use licenses, Ukrainian/English evidence, context/tool/structure support, hardware fit, and evaluation cost to shortlist models rather than chasing leaderboards.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M15-T04 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m15-t04: select multilingual local model candidates`.
