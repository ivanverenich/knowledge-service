---
id: M15-T16
title: "Pass the local ML understanding gate"
milestone: M15
track: optional
depends_on: ["M15-T15"]
estimate: 90m
topics: ["ollama","vllm","qlora"]
deliverables: ["M15 gate evidence"]
status: planned
---

# M15-T16 — Pass the local ML understanding gate

## Outcome

Explain embedding versus reranking versus generation tuning, update runtime diagrams, diagnose a compatibility failure, and change one serving parameter independently.
...
2. Explain embedding versus reranking versus generation tuning, update runtime diagrams, diagnose a compatibility failure, and change one serving parameter independently.
...
- Evidence includes environment separation, budget compliance, evaluation, and GPU teardown.
...
**Implementation prompt:** Inspect M15-T16, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Explain embedding versus reranking versus generation tuning, update runtime diagrams, diagnose a compatibility failure, and change one serving parameter independently.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M15-T16 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m15-t16: pass the local ml gate`.
