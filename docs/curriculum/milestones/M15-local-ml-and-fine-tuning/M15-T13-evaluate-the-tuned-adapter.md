---
id: M15-T13
title: "Evaluate the tuned adapter"
milestone: M15
track: optional
depends_on: ["M15-T12"]
estimate: 90m
topics: ["ollama","vllm","qlora"]
deliverables: ["fine-tuning comparison"]
status: planned
---

# M15-T13 — Evaluate the tuned adapter

## Outcome

Compare base, prompt/RAG, and tuned outputs on untouched bilingual cases for target behavior, regressions, citations, latency, and safety.
...
2. Compare base, prompt/RAG, and tuned outputs on untouched bilingual cases for target behavior, regressions, citations, latency, and safety.
...
- A blinded human sample and automated metrics support or reject the hypothesis.
...
**Implementation prompt:** Inspect M15-T13, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Compare base, prompt/RAG, and tuned outputs on untouched bilingual cases for target behavior, regressions, citations, latency, and safety.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M15-T13 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m15-t13: evaluate the tuned adapter`.
