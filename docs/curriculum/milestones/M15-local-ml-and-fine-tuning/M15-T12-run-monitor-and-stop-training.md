---
id: M15-T12
title: "Run, monitor, and stop training"
milestone: M15
track: optional
depends_on: ["M15-T11"]
estimate: 90m
topics: ["ollama","vllm","qlora"]
deliverables: ["training run artifacts"]
status: planned
---

# M15-T12 — Run, monitor, and stop training

## Outcome

Execute one bounded run, track loss/validation, GPU memory, throughput, checkpoints, spend, and stop conditions without exposing dataset content.
...
2. Execute one bounded run, track loss/validation, GPU memory, throughput, checkpoints, spend, and stop conditions without exposing dataset content.
...
- The run terminates within budget and produces reproducible adapter/checkpoint metadata.
...
**Implementation prompt:** Inspect M15-T12, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Execute one bounded run, track loss/validation, GPU memory, throughput, checkpoints, spend, and stop conditions without exposing dataset content.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M15-T12 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m15-t12: run monitor and stop training`.
