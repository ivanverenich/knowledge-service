...
deliverables: ["vLLM benchmark"]
status: planned
---

# M15-T07 — Benchmark vLLM serving

## Outcome

Measure throughput, time to first token, completion latency, batch behavior, memory, cancellation, and cost across controlled concurrency.
...
2. Measure throughput, time to first token, completion latency, batch behavior, memory, cancellation, and cost across controlled concurrency.
...
- Results compare vLLM with managed and Ollama paths using identical prompts where valid.
...
**Implementation prompt:** Inspect M15-T07, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Measure throughput, time to first token, completion latency, batch behavior, memory, cancellation, and cost across controlled concurrency.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M15-T07 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m15-t07: benchmark vllm serving`.
