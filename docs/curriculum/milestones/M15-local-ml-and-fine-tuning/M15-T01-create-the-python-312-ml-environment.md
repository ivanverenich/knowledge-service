---
id: M15-T01
title: "Create the Python 3.12 ML environment"
milestone: M15
track: optional
depends_on: ["M10-T18"]
estimate: 90m
topics: ["ollama","vllm","qlora"]
deliverables: ["ML project metadata","ML lockfile"]
status: planned
---

# M15-T01 — Create the Python 3.12 ML environment

## Outcome

Create a separately locked Python 3.12 project/container for PyTorch, Transformers, PEFT, evaluation, and later vLLM without lowering the application runtime.
...
2. Create a separately locked Python 3.12 project/container for PyTorch, Transformers, PEFT, evaluation, and later vLLM without lowering the application runtime.
...
- Both environments install reproducibly and communicate only through declared data/model interfaces.
...
**Implementation prompt:** Inspect M15-T01, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Create a separately locked Python 3.12 project/container for PyTorch, Transformers, PEFT, evaluation, and later vLLM without lowering the application runtime.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M15-T01 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m15-t01: create the python 312 ml environment`.
