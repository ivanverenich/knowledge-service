---
id: M14-T08
title: "Run migrations and maintenance Jobs"
milestone: M14
track: optional
depends_on: ["M14-T07"]
estimate: 90m
topics: ["kubernetes","helm","eks"]
deliverables: ["migration Job"]
status: planned
---

# M14-T08 — Run migrations and maintenance Jobs

## Outcome

Create idempotent pre-deployment migration and controlled maintenance Jobs with concurrency, retries, deadlines, and observability.
...
2. Create idempotent pre-deployment migration and controlled maintenance Jobs with concurrency, retries, deadlines, and observability.
...
- A failed migration blocks rollout and a retry does not corrupt schema state.
...
**Implementation prompt:** Inspect M14-T08, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Create idempotent pre-deployment migration and controlled maintenance Jobs with concurrency, retries, deadlines, and observability.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T08 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t08: run migrations and maintenance jobs`.
