---
id: M14-T07
title: "Add probes and graceful termination"
milestone: M14
track: optional
depends_on: ["M14-T06"]
estimate: 90m
topics: ["kubernetes","helm","eks"]
deliverables: ["probe/lifecycle configuration"]
status: planned
---

# M14-T07 — Add probes and graceful termination

## Outcome

Map liveness, readiness, startup, preStop, and termination grace to actual application lifecycle and long-running streams/jobs.
...
2. Map liveness, readiness, startup, preStop, and termination grace to actual application lifecycle and long-running streams/jobs.
...
- Probe and rollout drills avoid restart loops and allow bounded drain.
...
**Implementation prompt:** Inspect M14-T07, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Map liveness, readiness, startup, preStop, and termination grace to actual application lifecycle and long-running streams/jobs.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T07 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t07: add probes and graceful termination`.
