---
id: M14-T11
title: "Run Kubernetes failure drills"
milestone: M14
track: optional
depends_on: ["M14-T10"]
estimate: 90m
topics: ["kubernetes","helm","eks"]
deliverables: ["Kubernetes drill report"]
status: planned
---

# M14-T11 — Run Kubernetes failure drills

## Outcome

Kill pods, break readiness, exhaust resources, interrupt dependencies, and test rollout rollback with documented expected behavior.
...
2. Kill pods, break readiness, exhaust resources, interrupt dependencies, and test rollout rollback with documented expected behavior.
...
- Evidence distinguishes self-healing from repeated failure and preserves durable outcomes.
...
**Implementation prompt:** Inspect M14-T11, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Kill pods, break readiness, exhaust resources, interrupt dependencies, and test rollout rollback with documented expected behavior.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T11 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t11: run kubernetes failure drills`.
