---
id: M14-T10
title: "Integrate cluster observability"
milestone: M14
track: optional
depends_on: ["M14-T09"]
estimate: 90m
topics: ["kubernetes","helm","eks"]
deliverables: ["Kubernetes telemetry"]
status: planned
---

# M14-T10 — Integrate cluster observability

## Outcome

Export application and Kubernetes telemetry with correlation, resource attributes, dashboards, and redaction policy.
...
2. Export application and Kubernetes telemetry with correlation, resource attributes, dashboards, and redaction policy.
...
- One request/Job can be traced through pods while node/pod saturation is visible.
...
**Implementation prompt:** Inspect M14-T10, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Export application and Kubernetes telemetry with correlation, resource attributes, dashboards, and redaction policy.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T10 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t10: integrate cluster observability`.
