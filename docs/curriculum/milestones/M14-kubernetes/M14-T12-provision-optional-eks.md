---
id: M14-T12
title: "Provision optional EKS"
milestone: M14
track: optional
depends_on: ["M14-T11"]
estimate: 90m
topics: ["kubernetes","helm","eks"]
deliverables: ["EKS Terraform extension"]
status: planned
---

# M14-T12 — Provision optional EKS

## Outcome

Extend Terraform with a small ephemeral EKS environment, workload identity, private data access, ingress, autoscaling prerequisites, and explicit cost gates.
...
2. Extend Terraform with a small ephemeral EKS environment, workload identity, private data access, ingress, autoscaling prerequisites, and explicit cost gates.
...
- The chart deploys unchanged except environment values and IAM uses workload identity rather than static keys.
...
**Implementation prompt:** Inspect M14-T12, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Extend Terraform with a small ephemeral EKS environment, workload identity, private data access, ingress, autoscaling prerequisites, and explicit cost gates.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T12 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t12: provision optional eks`.
