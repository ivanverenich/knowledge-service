---
id: M14-T06
title: "Configure secrets and config"
milestone: M14
track: optional
depends_on: ["M14-T05"]
estimate: 75m
topics: ["kubernetes","helm","eks"]
deliverables: ["Kubernetes configuration policy"]
status: planned
---

# M14-T06 — Configure secrets and config

## Outcome

Separate non-secret ConfigMaps from secret-provider references, document local development secrets, and add rotation/restart behavior.
...
2. Separate non-secret ConfigMaps from secret-provider references, document local development secrets, and add rotation/restart behavior.
...
- Rendered manifests and Helm output contain no real secret values.
...
**Implementation prompt:** Inspect M14-T06, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Separate non-secret ConfigMaps from secret-provider references, document local development secrets, and add rotation/restart behavior.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T06 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t06: configure secrets and config`.
