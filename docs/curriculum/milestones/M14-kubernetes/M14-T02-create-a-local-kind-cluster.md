...
deliverables: ["kind cluster configuration"]
status: planned
---

# M14-T02 — Create a local kind cluster

## Outcome

Define a reproducible cluster with ingress/load-balancer support, resource-aware node settings, and deterministic teardown.
...
2. Define a reproducible cluster with ingress/load-balancer support, resource-aware node settings, and deterministic teardown.
...
- A new cluster passes diagnostics and deletion removes only the named learning cluster.
...
**Implementation prompt:** Inspect M14-T02, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Define a reproducible cluster with ingress/load-balancer support, resource-aware node settings, and deterministic teardown.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T02 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t02: create a local kind cluster`.
