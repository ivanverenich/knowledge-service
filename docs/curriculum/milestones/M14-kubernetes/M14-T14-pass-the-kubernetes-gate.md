---
id: M14-T14
title: "Pass the Kubernetes understanding gate"
milestone: M14
track: optional
depends_on: ["M14-T13"]
estimate: 90m
topics: ["kubernetes","helm","eks"]
deliverables: ["M14 gate evidence"]
status: planned
---

# M14-T14 — Pass the Kubernetes understanding gate

## Outcome

Explain controllers and desired state, update deployment diagrams, diagnose a probe loop, add one chart value independently, and tear clusters down.
...
2. Explain controllers and desired state, update deployment diagrams, diagnose a probe loop, add one chart value independently, and tear clusters down.
...
- Rendered manifests, drill evidence, and cost/teardown checks satisfy the gate.
...
**Implementation prompt:** Inspect M14-T14, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Explain controllers and desired state, update deployment diagrams, diagnose a probe loop, add one chart value independently, and tear clusters down.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M14-T14 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m14-t14: pass the kubernetes gate`.
