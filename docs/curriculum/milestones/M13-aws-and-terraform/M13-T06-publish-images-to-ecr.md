---
id: M13-T06
title: "Publish images to ECR"
milestone: M13
track: critical
depends_on: ["M13-T05"]
estimate: 75m
topics: ["aws","terraform","ecs"]
deliverables: ["ECR module"]
status: planned
---

# M13-T06 — Publish images to ECR

## Outcome

Provision immutable ECR repositories, scan-on-push, lifecycle retention, encryption, and CI authentication without long-lived keys.
...
2. Provision immutable ECR repositories, scan-on-push, lifecycle retention, encryption, and CI authentication without long-lived keys.
...
- CI publishes a digest-addressed scanned image and cannot overwrite immutable tags.
...
**Implementation prompt:** Inspect M13-T06, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Provision immutable ECR repositories, scan-on-push, lifecycle retention, encryption, and CI authentication without long-lived keys.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T06 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t06: publish images to ecr`.
