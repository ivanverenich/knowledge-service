---
id: M13-T17
title: "Run restore, load, and failure drills"
milestone: M13
track: critical
depends_on: ["M13-T16"]
estimate: 90m
topics: ["aws","terraform","ecs"]
deliverables: ["AWS operations drill report"]
status: planned
---

# M13-T17 — Run restore, load, and failure drills

## Outcome

Restore RDS into isolation, exercise ten-user-plus-ingestion load, stop tasks, interrupt Redis, and record recovery/SLO evidence.
...
2. Restore RDS into isolation, exercise ten-user-plus-ingestion load, stop tasks, interrupt Redis, and record recovery/SLO evidence.
...
- The report distinguishes application overhead, provider latency, saturation, RPO, and RTO.
...
**Implementation prompt:** Inspect M13-T17, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Restore RDS into isolation, exercise ten-user-plus-ingestion load, stop tasks, interrupt Redis, and record recovery/SLO evidence.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T17 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t17: run restore load and failure drills`.
