---
id: M13-T13
title: "Deploy OIDC for the reference environment"
milestone: M13
track: critical
depends_on: ["M13-T12"]
estimate: 90m
topics: ["aws","terraform","ecs"]
deliverables: ["Keycloak AWS deployment"]
status: planned
---

# M13-T13 — Deploy OIDC for the reference environment

## Outcome

Run the selected Keycloak reference deployment with private database, TLS issuer, backup scope, bootstrap-secret procedure, and portfolio-safe demo realm.
...
2. Run the selected Keycloak reference deployment with private database, TLS issuer, backup scope, bootstrap-secret procedure, and portfolio-safe demo realm.
...
- Token issuer/audience/JWKS work externally and administrative endpoints follow the intended exposure policy.
...
**Implementation prompt:** Inspect M13-T13, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Run the selected Keycloak reference deployment with private database, TLS issuer, backup scope, bootstrap-secret procedure, and portfolio-safe demo realm.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T13 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t13: deploy oidc for the reference environment`.
