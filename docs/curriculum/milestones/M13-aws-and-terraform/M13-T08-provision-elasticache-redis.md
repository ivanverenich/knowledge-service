---
id: M13-T08
title: "Provision ElastiCache Redis"
milestone: M13
track: critical
depends_on: ["M13-T07"]
estimate: 90m
topics: ["aws","terraform","ecs"]
deliverables: ["ElastiCache module"]
status: planned
---

# M13-T08 — Provision ElastiCache Redis

## Outcome

Create private encrypted Redis with authentication, subnet/security groups, maintenance settings, and a documented ephemeral-state recovery model.
...
2. Create private encrypted Redis with authentication, subnet/security groups, maintenance settings, and a documented ephemeral-state recovery model.
...
- Only API/worker tasks can connect and an interruption drill leaves durable work recoverable.
...
**Implementation prompt:** Inspect M13-T08, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Create private encrypted Redis with authentication, subnet/security groups, maintenance settings, and a documented ephemeral-state recovery model.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T08 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t08: provision elasticache redis`.
