---
id: M13-T02
title: "Bootstrap Terraform state"
milestone: M13
track: critical
depends_on: ["M13-T01"]
estimate: 90m
topics: ["aws","terraform","ecs"]
deliverables: ["Terraform backend"]
status: planned
---

# M13-T02 — Bootstrap Terraform state

## Outcome

Create a separately managed encrypted S3 backend and locking strategy with least privilege, recovery notes, and environment separation.
...
2. Create a separately managed encrypted S3 backend and locking strategy with least privilege, recovery notes, and environment separation.
...
- Concurrent plan attempts are serialized and state contains no application secrets.
...
**Implementation prompt:** Inspect M13-T02, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Create a separately managed encrypted S3 backend and locking strategy with least privilege, recovery notes, and environment separation.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T02 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t02: bootstrap terraform state`.
