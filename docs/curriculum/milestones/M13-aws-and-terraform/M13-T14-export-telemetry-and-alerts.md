---
id: M13-T14
title: "Export telemetry and alerts"
milestone: M13
track: critical
depends_on: ["M13-T13"]
estimate: 90m
topics: ["aws","terraform","ecs"]
deliverables: ["AWS telemetry","alarms"]
status: planned
---

# M13-T14 — Export telemetry and alerts

## Outcome

Route structured logs, OTel metrics/traces, and alarms for errors, latency, queue age, sync freshness, capacity, and security denials.
...
2. Route structured logs, OTel metrics/traces, and alarms for errors, latency, queue age, sync freshness, capacity, and security denials.
...
- A controlled failure reaches the intended dashboard/alarm without raw content.
...
**Implementation prompt:** Inspect M13-T14, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Route structured logs, OTel metrics/traces, and alarms for errors, latency, queue age, sync freshness, capacity, and security denials.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T14 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t14: export telemetry and alerts`.
