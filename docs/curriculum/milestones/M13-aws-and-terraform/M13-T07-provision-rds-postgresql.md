...
deliverables: ["RDS module"]
status: planned
---

# M13-T07 — Provision RDS PostgreSQL

## Outcome

Create private encrypted PostgreSQL with pgvector-compatible engine choice, parameter/backup settings, secret reference, monitoring, and deletion protection policy.
...
2. Create private encrypted PostgreSQL with pgvector-compatible engine choice, parameter/backup settings, secret reference, monitoring, and deletion protection policy.
...
- Connectivity works only from intended tasks; migration and restore prerequisites are validated.
...
**Implementation prompt:** Inspect M13-T07, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Create private encrypted PostgreSQL with pgvector-compatible engine choice, parameter/backup settings, secret reference, monitoring, and deletion protection policy.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T07 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t07: provision rds postgresql`.
