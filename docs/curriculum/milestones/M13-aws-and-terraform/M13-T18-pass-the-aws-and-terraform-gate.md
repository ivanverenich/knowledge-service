---
id: M13-T18
title: "Pass the AWS and Terraform understanding gate"
milestone: M13
track: critical
depends_on: ["M13-T17"]
estimate: 90m
topics: ["aws","terraform","ecs"]
deliverables: ["M13 gate evidence"]
status: planned
---

# M13-T18 — Pass the AWS and Terraform understanding gate

## Outcome

Explain trust/network/state boundaries, update the deployment diagram, diagnose an injected IAM failure, add one alarm, then destroy the environment safely.
...
2. Explain trust/network/state boundaries, update the deployment diagram, diagnose an injected IAM failure, add one alarm, then destroy the environment safely.
...
- Evidence includes clean Terraform state, retained approved artifacts, cost review, and confirmed teardown.
...
**Implementation prompt:** Inspect M13-T18, prerequisites, current interfaces, and cost/security constraints. Propose a bounded plan for: “Explain trust/network/state boundaries, update the deployment diagram, diagnose an injected IAM failure, add one alarm, then destroy the environment safely.” Wait for approval; report changes, exact validation, spend, and teardown.

**Review prompt:** Review M13-T18 for state/secret exposure, least privilege, version drift, cost risk, cleanup, reproducibility, evaluation validity, and interface leakage. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance and teardown/rollback evidence exists, using intent: `m13-t18: pass the aws and terraform gate`.
