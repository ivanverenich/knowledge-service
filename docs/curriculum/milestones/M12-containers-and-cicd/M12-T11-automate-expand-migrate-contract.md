---
id: M12-T11
title: "Automate expand-migrate-contract"
milestone: M12
track: critical
depends_on: ["M12-T10"]
estimate: 90m
topics: ["docker","ci-cd","supply-chain"]
deliverables: ["migration deployment workflow"]
status: planned
---

# M12-T11 — Automate expand-migrate-contract

## Outcome

Create deployment stages for backward-compatible schema expansion, data migration, application rollout, and delayed contraction.
...
2. Create deployment stages for backward-compatible schema expansion, data migration, application rollout, and delayed contraction.
...
- A rehearsal keeps old and new application versions functional through the migration window.
...
**Implementation prompt:** Inspect M12-T11, prerequisites, and current interfaces. Propose a bounded plan for: “Create deployment stages for backward-compatible schema expansion, data migration, application rollout, and delayed contraction.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M12-T11 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m12-t11: automate expand migrate contract`.
