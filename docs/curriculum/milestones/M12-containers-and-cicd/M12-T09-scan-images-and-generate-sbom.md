---
id: M12-T09
title: "Scan images and generate SBOM"
milestone: M12
track: critical
depends_on: ["M12-T08"]
estimate: 90m
topics: ["docker","ci-cd","supply-chain"]
deliverables: ["SBOM","container scan"]
status: planned
---

# M12-T09 — Scan images and generate SBOM

## Outcome

Produce an SBOM, scan final images, record base digest, and attach artifacts to an immutable image identifier.
...
2. Produce an SBOM, scan final images, record base digest, and attach artifacts to an immutable image identifier.
...
- A release candidate has traceable packages and fails on the configured critical finding policy.
...
**Implementation prompt:** Inspect M12-T09, prerequisites, and current interfaces. Propose a bounded plan for: “Produce an SBOM, scan final images, record base digest, and attach artifacts to an immutable image identifier.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M12-T09 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m12-t09: scan images and generate sbom`.
