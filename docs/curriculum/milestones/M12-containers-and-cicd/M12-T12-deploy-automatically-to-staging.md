---
id: M12-T12
title: "Deploy automatically to staging"
milestone: M12
track: critical
depends_on: ["M12-T11"]
estimate: 90m
topics: ["docker","ci-cd","supply-chain"]
deliverables: ["staging deployment workflow"]
status: planned
---

# M12-T12 — Deploy automatically to staging

## Outcome

Promote an immutable tested image to staging with concurrency control, smoke tests, migration checks, and environment evidence.
...
2. Promote an immutable tested image to staging with concurrency control, smoke tests, migration checks, and environment evidence.
...
- Two rapid merges cannot overlap deployment and a failed smoke test blocks promotion.
...
**Implementation prompt:** Inspect M12-T12, prerequisites, and current interfaces. Propose a bounded plan for: “Promote an immutable tested image to staging with concurrency control, smoke tests, migration checks, and environment evidence.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M12-T12 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m12-t12: deploy automatically to staging`.
