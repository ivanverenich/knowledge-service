---
id: M12-T04
title: "Implement health, startup, and shutdown"
milestone: M12
track: critical
depends_on: ["M12-T03"]
estimate: 90m
topics: ["docker","ci-cd","supply-chain"]
deliverables: ["runtime lifecycle policy"]
status: planned
---

# M12-T04 — Implement health, startup, and shutdown

## Outcome

Make readiness reflect required dependencies, avoid fragile startup ordering, and close HTTP/database/telemetry resources within termination grace.
...
2. Make readiness reflect required dependencies, avoid fragile startup ordering, and close HTTP/database/telemetry resources within termination grace.
...
- Failure drills show liveness stays meaningful and no new work enters an unready process.
...
**Implementation prompt:** Inspect M12-T04, prerequisites, and current interfaces. Propose a bounded plan for: “Make readiness reflect required dependencies, avoid fragile startup ordering, and close HTTP/database/telemetry resources within termination grace.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M12-T04 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m12-t04: implement health startup and shutdown`.
