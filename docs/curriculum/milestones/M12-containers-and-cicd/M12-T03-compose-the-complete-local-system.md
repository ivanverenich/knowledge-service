---
id: M12-T03
title: "Compose the complete local system"
milestone: M12
track: critical
depends_on: ["M12-T02"]
estimate: 90m
topics: ["docker","ci-cd","supply-chain"]
deliverables: ["Docker Compose stack"]
status: planned
---

# M12-T03 — Compose the complete local system

## Outcome

Run API, worker, PostgreSQL/pgvector, Redis, Keycloak, Open WebUI, and observability with named health dependencies and isolated persistent volumes.
...
2. Run API, worker, PostgreSQL/pgvector, Redis, Keycloak, Open WebUI, and observability with named health dependencies and isolated persistent volumes.
...
- A clean machine follows one guide to reach a cited bilingual Answer.
...
**Implementation prompt:** Inspect M12-T03, prerequisites, and current interfaces. Propose a bounded plan for: “Run API, worker, PostgreSQL/pgvector, Redis, Keycloak, Open WebUI, and observability with named health dependencies and isolated persistent volumes.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M12-T03 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m12-t03: compose the complete local system`.
