---
id: M01-T04
title: "Implement the OpenAI chat adapter"
milestone: M01
track: critical
depends_on: ["M01-T03"]
estimate: 90m
...
Implement the first managed adapter with explicit timeouts, model configuration, request identifiers, usage capture, and provider-error translation.
...
2. Implement the first managed adapter with explicit timeouts, model configuration, request identifiers, usage capture, and provider-error translation.
...
- Contract tests with a fake HTTP boundary cover success, timeout, rate limit, malformed output, and cancellation.
...
**Implementation prompt:** Inspect M01-T04 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Implement the first managed adapter with explicit timeouts, model configuration, request identifiers, usage capture, and provider-error translation.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M01-T04 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m01-t04: implement the openai chat adapter`.
