---
id: M01-T11
title: "Write walking-skeleton contract tests"
milestone: M01
track: critical
depends_on: ["M01-T10"]
estimate: 75m
...
Run the same ChatModel behavior suite against the fake and managed adapter boundary while keeping live calls opt-in.
...
2. Run the same ChatModel behavior suite against the fake and managed adapter boundary while keeping live calls opt-in.
...
- The deterministic suite is credential-free and the opt-in marker cannot run accidentally in CI.
...
**Implementation prompt:** Inspect M01-T11 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Run the same ChatModel behavior suite against the fake and managed adapter boundary while keeping live calls opt-in.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M01-T11 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m01-t11: write walking skeleton contract tests`.
