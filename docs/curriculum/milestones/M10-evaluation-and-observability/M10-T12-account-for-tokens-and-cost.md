---
id: M10-T12
title: "Account for tokens and cost"
milestone: M10
track: critical
depends_on: ["M10-T11"]
estimate: 75m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["cost accounting"]
status: planned
---

# M10-T12 — Account for tokens and cost

## Outcome

Aggregate embedding, reranking, generation, judge, and agent usage using versioned configurable price data and unknown-price handling.
...
2. Aggregate embedding, reranking, generation, judge, and agent usage using versioned configurable price data and unknown-price handling.
...
- Reports separate service overhead from provider spend and enforce a run ceiling.
...
**Implementation prompt:** Inspect M10-T12, its dependencies, and current evidence. Propose a bounded plan for: “Aggregate embedding, reranking, generation, judge, and agent usage using versioned configurable price data and unknown-price handling.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T12 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t12: account for tokens and cost`.
