---
id: M10-T13
title: "Establish regression thresholds"
milestone: M10
track: critical
depends_on: ["M10-T12"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["quality gate configuration","baseline report"]
status: planned
---

# M10-T13 — Establish regression thresholds

## Outcome

Measure the named baseline, define per-slice quality tolerances, zero-leak security gate, latency targets, and statistical comparison policy.
...
2. Measure the named baseline, define per-slice quality tolerances, zero-leak security gate, latency targets, and statistical comparison policy.
...
- A deliberately degraded configuration fails the intended release gates.
...
**Implementation prompt:** Inspect M10-T13, its dependencies, and current evidence. Propose a bounded plan for: “Measure the named baseline, define per-slice quality tolerances, zero-leak security gate, latency targets, and statistical comparison policy.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T13 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t13: establish regression thresholds`.
