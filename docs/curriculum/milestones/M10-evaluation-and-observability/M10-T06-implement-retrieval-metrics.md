---
id: M10-T06
title: "Implement retrieval metrics"
milestone: M10
track: critical
depends_on: ["M10-T05"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["retrieval scorers"]
status: planned
---

# M10-T06 — Implement retrieval metrics

## Outcome

Calculate Recall@k, MRR, and nDCG with stable relevance labels, empty behavior, and per-slice aggregation.
...
2. Calculate Recall@k, MRR, and nDCG with stable relevance labels, empty behavior, and per-slice aggregation.
...
- Known toy rankings produce hand-calculated expected scores.
...
**Implementation prompt:** Inspect M10-T06, its dependencies, and current evidence. Propose a bounded plan for: “Calculate Recall@k, MRR, and nDCG with stable relevance labels, empty behavior, and per-slice aggregation.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T06 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t06: implement retrieval metrics`.
