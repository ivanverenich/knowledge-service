---
id: M10-T16
title: "Load-test chat with concurrent ingestion"
milestone: M10
track: critical
depends_on: ["M10-T15"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["load-test scenario","performance report"]
status: planned
---

# M10-T16 — Load-test chat with concurrent ingestion

## Outcome

Model ten concurrent streaming users plus background synchronization, warmup, realistic think time, cancellation, and provider isolation.
...
2. Model ten concurrent streaming users plus background synchronization, warmup, realistic think time, cancellation, and provider isolation.
...
- Report p50/p95/p99 retrieval and time-to-first-token, errors, saturation, and bottleneck evidence.
...
**Implementation prompt:** Inspect M10-T16, its dependencies, and current evidence. Propose a bounded plan for: “Model ten concurrent streaming users plus background synchronization, warmup, realistic think time, cancellation, and provider isolation.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T16 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t16: load test chat with concurrent ingestion`.
