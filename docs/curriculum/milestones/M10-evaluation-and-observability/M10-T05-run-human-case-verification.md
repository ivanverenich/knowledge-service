---
id: M10-T05
title: "Run human case verification"
milestone: M10
track: critical
depends_on: ["M10-T04"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["verification rubric","verified dataset"]
status: planned
---

# M10-T05 — Run human case verification

## Outcome

Create a review workflow that checks Question naturalness, expected Evidence, claims, permissions, language labels, and split assignment.
...
2. Create a review workflow that checks Question naturalness, expected Evidence, claims, permissions, language labels, and split assignment.
...
- Only reviewed cases enter the approximately 200-case released dataset.
...
**Implementation prompt:** Inspect M10-T05, its dependencies, and current evidence. Propose a bounded plan for: “Create a review workflow that checks Question naturalness, expected Evidence, claims, permissions, language labels, and split assignment.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T05 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t05: run human case verification`.
