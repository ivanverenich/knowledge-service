---
id: M10-T15
title: "Build local dashboards and alerts"
milestone: M10
track: critical
depends_on: ["M10-T14"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["OTel collector config","dashboards","alerts"]
status: planned
---

# M10-T15 — Build local dashboards and alerts

## Outcome

Configure collector/exporters and dashboards for request/error/latency, queue depth/age, sync freshness, retrieval stages, tokens/cost, and security denials.
...
2. Configure collector/exporters and dashboards for request/error/latency, queue depth/age, sync freshness, retrieval stages, tokens/cost, and security denials.
...
- A scripted scenario changes each key panel and triggers one test alert.
...
**Implementation prompt:** Inspect M10-T15, its dependencies, and current evidence. Propose a bounded plan for: “Configure collector/exporters and dashboards for request/error/latency, queue depth/age, sync freshness, retrieval stages, tokens/cost, and security denials.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T15 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t15: build local dashboards and alerts`.
