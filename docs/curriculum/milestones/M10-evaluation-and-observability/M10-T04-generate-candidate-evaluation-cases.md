---
id: M10-T04
title: "Generate candidate Evaluation Cases"
milestone: M10
track: critical
depends_on: ["M10-T03"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["case-generation tool","draft cases"]
status: planned
---

# M10-T04 — Generate candidate Evaluation Cases

## Outcome

Use a versioned prompt/tool to propose bilingual, cross-language, multi-hop, unanswerable, identifier, table, and adversarial cases.
...
2. Use a versioned prompt/tool to propose bilingual, cross-language, multi-hop, unanswerable, identifier, table, and adversarial cases.
...
- Generation output remains untrusted draft data with model and prompt provenance.
...
**Implementation prompt:** Inspect M10-T04, its dependencies, and current evidence. Propose a bounded plan for: “Use a versioned prompt/tool to propose bilingual, cross-language, multi-hop, unanswerable, identifier, table, and adversarial cases.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T04 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t04: generate candidate evaluation cases`.
