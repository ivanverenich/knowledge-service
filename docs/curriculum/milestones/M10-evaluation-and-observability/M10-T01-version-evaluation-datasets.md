---
id: M10-T01
title: "Version Evaluation Datasets"
milestone: M10
track: critical
depends_on: ["M09-T12"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["evaluation dataset schema"]
status: planned
---

# M10-T01 — Version Evaluation Datasets

## Outcome

Define immutable dataset metadata, case schema, lineage, splits, authorization context, language direction, expected evidence, claims, and answerability.
...
2. Define immutable dataset metadata, case schema, lineage, splits, authorization context, language direction, expected evidence, claims, and answerability.
...
- Schema validation rejects leakage between splits and incomplete security cases.
...
**Implementation prompt:** Inspect M10-T01, its dependencies, and current evidence. Propose a bounded plan for: “Define immutable dataset metadata, case schema, lineage, splits, authorization context, language direction, expected evidence, claims, and answerability.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T01 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t01: version evaluation datasets`.
