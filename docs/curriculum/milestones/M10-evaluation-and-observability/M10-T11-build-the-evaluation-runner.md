---
id: M10-T11
title: "Build the Evaluation Runner"
milestone: M10
track: critical
depends_on: ["M10-T10"]
estimate: 90m
topics: ["evaluation","opentelemetry","load-testing"]
deliverables: ["evaluation runner"]
status: planned
---

# M10-T11 — Build the Evaluation Runner

## Outcome

Execute a frozen dataset/configuration with bounded concurrency, resume, per-case artifacts, aggregation, cancellation, and run fingerprint.
...
2. Execute a frozen dataset/configuration with bounded concurrency, resume, per-case artifacts, aggregation, cancellation, and run fingerprint.
...
- Interrupted runs resume without double charging or mixing configurations.
...
**Implementation prompt:** Inspect M10-T11, its dependencies, and current evidence. Propose a bounded plan for: “Execute a frozen dataset/configuration with bounded concurrency, resume, per-case artifacts, aggregation, cancellation, and run fingerprint.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M10-T11 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m10-t11: build the evaluation runner`.
