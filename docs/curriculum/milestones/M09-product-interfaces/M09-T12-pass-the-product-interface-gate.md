---
id: M09-T12
title: "Pass the product-interface understanding gate"
milestone: M09
track: critical
depends_on: ["M09-T11"]
estimate: 90m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["M09 gate evidence"]
status: planned
---

# M09-T12 — Pass the product-interface understanding gate

## Outcome

Explain native versus compatible interfaces, redraw adapter flow, diagnose a broken stream, and add one native field without breaking compatibility.
...
2. Explain native versus compatible interfaces, redraw adapter flow, diagnose a broken stream, and add one native field without breaking compatibility.
...
- Evidence proves both transports still invoke one application workflow.
...
**Implementation prompt:** Inspect M09-T12, its dependencies, and current evidence. Propose a bounded plan for: “Explain native versus compatible interfaces, redraw adapter flow, diagnose a broken stream, and add one native field without breaking compatibility.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T12 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m09-t12: pass the product interface gate`.
