---
id: M09-T02
title: "Implement Source administration"
milestone: M09
track: critical
depends_on: ["M09-T01"]
estimate: 90m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["Source administration routes"]
status: planned
---

# M09-T02 — Implement Source administration

## Outcome

Expose create/update/enable/disable/list operations using secret references, validation, Application Roles, and audit events.
...
2. Expose create/update/enable/disable/list operations using secret references, validation, Application Roles, and audit events.
...
- HTTP tests cover role denial, duplicate identity, invalid config, secret redaction, and pagination.
...
**Implementation prompt:** Inspect M09-T02, its dependencies, and current evidence. Propose a bounded plan for: “Expose create/update/enable/disable/list operations using secret references, validation, Application Roles, and audit events.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T02 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m09-t02: implement source administration`.
