---
id: M09-T07
title: "Preserve Citations through compatibility"
milestone: M09
track: critical
depends_on: ["M09-T06"]
estimate: 75m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["citation compatibility mapping"]
status: planned
---

# M09-T07 — Preserve Citations through compatibility

## Outcome

Choose and document a compatible citation rendering that preserves human links while native structured Citations remain authoritative.
...
2. Choose and document a compatible citation rendering that preserves human links while native structured Citations remain authoritative.
...
- Open WebUI-visible output resolves only validated authorized sources.
...
**Implementation prompt:** Inspect M09-T07, its dependencies, and current evidence. Propose a bounded plan for: “Choose and document a compatible citation rendering that preserves human links while native structured Citations remain authoritative.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T07 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m09-t07: preserve citations through compatibility`.
