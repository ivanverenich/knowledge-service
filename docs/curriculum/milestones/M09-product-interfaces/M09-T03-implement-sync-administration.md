---
id: M09-T03
title: "Implement synchronization administration"
milestone: M09
track: critical
depends_on: ["M09-T02"]
estimate: 90m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["sync administration routes"]
status: planned
---

# M09-T03 — Implement synchronization administration

## Outcome

Expose start, status, cancel, retry, and history operations over durable runs/jobs rather than broker internals.
...
2. Expose start, status, cancel, retry, and history operations over durable runs/jobs rather than broker internals.
...
- Contract tests prove idempotent start, ownership, terminal states, and stable error responses.
...
**Implementation prompt:** Inspect M09-T03, its dependencies, and current evidence. Propose a bounded plan for: “Expose start, status, cancel, retry, and history operations over durable runs/jobs rather than broker internals.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T03 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m09-t03: implement sync administration`.
