---
id: M09-T01
title: "Design native resource routes"
milestone: M09
track: critical
depends_on: ["M08-T16"]
estimate: 75m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["native API route design"]
status: planned
---

# M09-T01 — Design native resource routes

## Outcome

Specify versioned routes for Questions, Conversations, Sources, synchronization, evaluation, feedback, and diagnostics with role requirements.
...
2. Specify versioned routes for Questions, Conversations, Sources, synchronization, evaluation, feedback, and diagnostics with role requirements.
...
- OpenAPI review shows consistent IDs, pagination, errors, and no provider-shaped domain models.
...
**Implementation prompt:** Inspect M09-T01, its dependencies, and current evidence. Propose a bounded plan for: “Specify versioned routes for Questions, Conversations, Sources, synchronization, evaluation, feedback, and diagnostics with role requirements.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T01 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m09-t01: design native resource routes`.
