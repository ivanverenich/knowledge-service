---
id: M09-T04
title: "Serve OpenAI-compatible models"
milestone: M09
track: critical
depends_on: ["M09-T03"]
estimate: 60m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["models compatibility endpoint"]
status: planned
---

# M09-T04 — Serve OpenAI-compatible models

## Outcome

Implement `/v1/models` with one virtual knowledge model and stable ownership/metadata expected by Open WebUI.
...
2. Implement `/v1/models` with one virtual knowledge model and stable ownership/metadata expected by Open WebUI.
...
- A protocol test validates shape, bearer authentication, and configured model visibility.
...
**Implementation prompt:** Inspect M09-T04, its dependencies, and current evidence. Propose a bounded plan for: “Implement `/v1/models` with one virtual knowledge model and stable ownership/metadata expected by Open WebUI.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T04 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m09-t04: serve openai compatible models`.
