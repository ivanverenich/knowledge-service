---
id: M09-T10
title: "Apply identity-aware rate limits"
milestone: M09
track: critical
depends_on: ["M09-T09"]
estimate: 90m
topics: ["rest","openai-compatible","open-webui"]
deliverables: ["rate-limit policy"]
status: planned
---

# M09-T10 — Apply identity-aware rate limits

## Outcome

Rate-limit Questions, expensive administration, and agent workflows by verified identity and operation with safe Redis failure behavior.
...
2. Rate-limit Questions, expensive administration, and agent workflows by verified identity and operation with safe Redis failure behavior.
...
- Clock-controlled tests verify bursts, recovery, role-specific limits, and no identity spoofing.
...
**Implementation prompt:** Inspect M09-T10, its dependencies, and current evidence. Propose a bounded plan for: “Rate-limit Questions, expensive administration, and agent workflows by verified identity and operation with safe Redis failure behavior.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M09-T10 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.
