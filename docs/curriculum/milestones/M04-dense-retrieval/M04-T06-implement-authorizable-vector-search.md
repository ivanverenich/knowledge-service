---
id: M04-T06
title: "Implement authorizable vector search"
milestone: M04
track: critical
depends_on: ["M04-T05"]
estimate: 90m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["vector-search query"]
status: planned
---
...
Query nearest Chunk vectors through a parameterized predicate slot designed to include availability and Access Grants before ranking.
...
2. Query nearest Chunk vectors through a parameterized predicate slot designed to include availability and Access Grants before ranking.
...
- Integration tests prove filtering occurs in SQL and return stable provenance with distance.
...
**Implementation prompt:** Inspect M04-T06 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Query nearest Chunk vectors through a parameterized predicate slot designed to include availability and Access Grants before ranking.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T06 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m04-t06: implement authorizable vector search`.
