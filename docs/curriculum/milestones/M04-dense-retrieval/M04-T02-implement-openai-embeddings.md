---
id: M04-T02
title: "Implement OpenAI embeddings"
milestone: M04
track: critical
depends_on: ["M04-T01"]
estimate: 90m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["OpenAI embedding adapter"]
status: planned
---

# M04-T02 — Implement OpenAI embeddings

## Outcome

Add the managed embedding adapter with configured model/dimensions, batching, deadlines, retry classification, usage, and safe logging.
...
2. Add the managed embedding adapter with configured model/dimensions, batching, deadlines, retry classification, usage, and safe logging.
...
- Contract tests cover ordering, token-limit rejection, rate limits, malformed dimensions, and cancellation.
...
**Implementation prompt:** Inspect M04-T02 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Add the managed embedding adapter with configured model/dimensions, batching, deadlines, retry classification, usage, and safe logging.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T02 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m04-t02: implement openai embeddings`.
