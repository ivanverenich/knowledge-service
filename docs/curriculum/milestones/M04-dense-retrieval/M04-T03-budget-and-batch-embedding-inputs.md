---
id: M04-T03
title: "Budget and batch embedding inputs"
milestone: M04
track: critical
depends_on: ["M04-T02"]
estimate: 75m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["embedding batcher"]
status: planned
---

# M04-T03 — Budget and batch embedding inputs

## Outcome

Create deterministic batching by item and token limits, bounded concurrency, backpressure, and resumable per-batch outcomes.
...
2. Create deterministic batching by item and token limits, bounded concurrency, backpressure, and resumable per-batch outcomes.
...
- Property tests show no lost/reordered Chunk and concurrency never exceeds configuration.
...
**Implementation prompt:** Inspect M04-T03 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Create deterministic batching by item and token limits, bounded concurrency, backpressure, and resumable per-batch outcomes.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T03 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m04-t03: budget and batch embedding inputs`.
