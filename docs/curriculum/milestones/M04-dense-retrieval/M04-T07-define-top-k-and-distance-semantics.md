---
id: M04-T07
title: "Define top-k and distance semantics"
milestone: M04
track: critical
depends_on: ["M04-T06"]
estimate: 60m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["retrieval result contract"]
status: planned
---
...
Normalize configuration and results so callers understand candidate limits, distance direction, tie-breaking, and empty behavior.
...
2. Normalize configuration and results so callers understand candidate limits, distance direction, tie-breaking, and empty behavior.
...
- Table-driven tests cover ties, fewer-than-k results, invalid k, and deterministic ordering.
...
**Implementation prompt:** Inspect M04-T07 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Normalize configuration and results so callers understand candidate limits, distance direction, tie-breaking, and empty behavior.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T07 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m04-t07: define top k and distance semantics`.
