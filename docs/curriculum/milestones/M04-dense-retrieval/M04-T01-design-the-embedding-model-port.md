---
id: M04-T01
title: "Design the EmbeddingModel port"
milestone: M04
track: critical
depends_on: ["M03-T14"]
estimate: 90m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["EmbeddingModel port","fake embedding adapter"]
status: planned
---

# M04-T01 — Design the EmbeddingModel port

## Outcome

Define a batch-oriented async embedding interface with input limits, dimensions, model identity, usage, partial-failure policy, and typed errors.
...
2. Define a batch-oriented async embedding interface with input limits, dimensions, model identity, usage, partial-failure policy, and typed errors.
...
- A deterministic fake proves stable vectors, batch boundaries, validation, and failure translation.
...
**Implementation prompt:** Inspect M04-T01 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Define a batch-oriented async embedding interface with input limits, dimensions, model identity, usage, partial-failure policy, and typed errors.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T01 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m04-t01: design the embedding model port`.
