---
id: M04-T08
title: "Add retrieval metadata filters"
milestone: M04
track: critical
depends_on: ["M04-T07"]
estimate: 90m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["metadata-filter model"]
status: planned
---

# M04-T08 — Add retrieval metadata filters

## Outcome

Support Source, content type, language, and recency filters with an allowlisted typed expression rather than arbitrary SQL fragments.
...
2. Support Source, content type, language, and recency filters with an allowlisted typed expression rather than arbitrary SQL fragments.
...
- Integration tests combine each filter with vector ranking and reject unsupported operators.
...
**Implementation prompt:** Inspect M04-T08 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Support Source, content type, language, and recency filters with an allowlisted typed expression rather than arbitrary SQL fragments.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T08 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.
