---
id: M04-T04
title: "Enable pgvector and vector schema"
milestone: M04
track: critical
depends_on: ["M04-T03"]
estimate: 90m
topics: ["embeddings","pgvector","retrieval"]
deliverables: ["pgvector migration","embedding table"]
status: planned
---

# M04-T04 — Enable pgvector and vector schema

## Outcome

Add a migration for the extension and versioned Chunk embeddings with dimension/model constraints and current-state linkage.
...
2. Add a migration for the extension and versioned Chunk embeddings with dimension/model constraints and current-state linkage.
...
- Migration tests reject wrong dimensions and preserve multiple embedding versions during transition.
...
**Implementation prompt:** Inspect M04-T04 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Add a migration for the extension and versioned Chunk embeddings with dimension/model constraints and current-state linkage.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M04-T04 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m04-t04: enable pgvector and vector schema`.
