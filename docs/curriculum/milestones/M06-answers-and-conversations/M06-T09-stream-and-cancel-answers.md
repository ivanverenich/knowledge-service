---
id: M06-T09
title: "Stream and cancel Answers"
milestone: M06
track: critical
depends_on: ["M06-T08"]
estimate: 90m
topics: ["generation","citations","sse"]
deliverables: ["streaming endpoint"]
status: planned
---

# M06-T09 — Stream and cancel Answers

## Outcome

Expose SSE while preserving backpressure, client cancellation, deadline handling, final usage, and cleanup of provider streams.
...
2. Expose SSE while preserving backpressure, client cancellation, deadline handling, final usage, and cleanup of provider streams.
...
- An HTTP test disconnects midstream and proves upstream work and spans terminate.
...
**Implementation prompt:** Inspect M06-T09 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Expose SSE while preserving backpressure, client cancellation, deadline handling, final usage, and cleanup of provider streams.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T09 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t09: stream and cancel answers`.
