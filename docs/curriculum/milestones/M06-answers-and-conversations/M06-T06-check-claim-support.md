...
deliverables: ["support-check module"]
status: planned
---

# M06-T06 — Check claim support

## Outcome

Introduce a versioned support-check interface that maps answer claims to cited excerpts and reports unsupported claims without silently rewriting history.
...
2. Introduce a versioned support-check interface that maps answer claims to cited excerpts and reports unsupported claims without silently rewriting history.
...
- A deterministic fake and optional judge adapter produce auditable structured findings.
...
**Implementation prompt:** Inspect M06-T06 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Introduce a versioned support-check interface that maps answer claims to cited excerpts and reports unsupported claims without silently rewriting history.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T06 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t06: check claim support`.
