---
id: M06-T05
title: "Validate Citation identities"
milestone: M06
track: critical
depends_on: ["M06-T04"]
estimate: 75m
topics: ["generation","citations","sse"]
deliverables: ["citation validator"]
status: planned
---

# M06-T05 — Validate Citation identities

## Outcome

Accept only Evidence IDs supplied to generation, deduplicate citations, and map them to current Document provenance server-side.
...
2. Accept only Evidence IDs supplied to generation, deduplicate citations, and map them to current Document provenance server-side.
...
- Adversarial tests prove invented or unauthorized IDs never reach the client.
...
**Implementation prompt:** Inspect M06-T05 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Accept only Evidence IDs supplied to generation, deduplicate citations, and map them to current Document provenance server-side.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T05 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t05: validate citation identities`.
