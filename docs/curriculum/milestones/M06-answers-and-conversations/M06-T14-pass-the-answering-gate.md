---
id: M06-T14
title: "Pass the answering understanding gate"
milestone: M06
track: critical
depends_on: ["M06-T13"]
estimate: 90m
topics: ["generation","citations","sse"]
deliverables: ["M06 gate evidence"]
status: planned
---

# M06-T14 — Pass the answering understanding gate

## Outcome

Explain Answer evidence flow, update the sequence diagram, diagnose a fake Citation, and add one SSE event without a recipe.
...
2. Explain Answer evidence flow, update the sequence diagram, diagnose a fake Citation, and add one SSE event without a recipe.
...
- Evidence proves validation and client reconstruction stayed compatible.
...
**Implementation prompt:** Inspect M06-T14 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Explain Answer evidence flow, update the sequence diagram, diagnose a fake Citation, and add one SSE event without a recipe.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T14 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t14: pass the answering gate`.
