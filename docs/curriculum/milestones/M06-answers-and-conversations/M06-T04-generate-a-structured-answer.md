---
id: M06-T04
title: "Generate a structured Answer"
milestone: M06
track: critical
depends_on: ["M06-T03"]
estimate: 90m
topics: ["generation","citations","sse"]
deliverables: ["answer-generation module"]
status: planned
---

# M06-T04 — Generate a structured Answer

## Outcome

Request answer text, citation IDs, abstention state, and reason through the ChatModel port and translate validation failures safely.
...
2. Request answer text, citation IDs, abstention state, and reason through the ChatModel port and translate validation failures safely.
...
- Workflow tests cover supported answer, refusal, invalid schema, and provider failure.
...
**Implementation prompt:** Inspect M06-T04 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Request answer text, citation IDs, abstention state, and reason through the ChatModel port and translate validation failures safely.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T04 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t04: generate a structured answer`.
