---
id: M06-T11
title: "Construct bounded history"
milestone: M06
track: critical
depends_on: ["M06-T10"]
estimate: 90m
topics: ["generation","citations","sse"]
deliverables: ["history builder"]
status: planned
---

# M06-T11 — Construct bounded history

## Outcome

Select recent and summarized turns under a token budget, exclude failed/unauthorized content, and retain references needed for follow-ups.
...
2. Select recent and summarized turns under a token budget, exclude failed/unauthorized content, and retain references needed for follow-ups.
...
- Tests cover long conversations, model changes, deletion, and permission revocation.
...
**Implementation prompt:** Inspect M06-T11 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Select recent and summarized turns under a token budget, exclude failed/unauthorized content, and retain references needed for follow-ups.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T11 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.
