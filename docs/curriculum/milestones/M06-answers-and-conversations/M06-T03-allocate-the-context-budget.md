---
id: M06-T03
title: "Allocate the context budget"
milestone: M06
track: critical
depends_on: ["M06-T02"]
estimate: 90m
topics: ["generation","citations","sse"]
deliverables: ["context-budget policy"]
status: planned
---

# M06-T03 — Allocate the context budget

## Outcome

Reserve tokens for policy, bounded history, Question, Evidence, and Answer; trim by explicit priority while preserving provenance.
...
2. Reserve tokens for policy, bounded history, Question, Evidence, and Answer; trim by explicit priority while preserving provenance.
...
- Boundary tests prove budgets are never exceeded and trimming decisions are reported.
...
**Implementation prompt:** Inspect M06-T03 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Reserve tokens for policy, bounded history, Question, Evidence, and Answer; trim by explicit priority while preserving provenance.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T03 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t03: allocate the context budget`.
