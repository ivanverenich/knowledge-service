...
deliverables: ["stream failure policy"]
status: planned
---

# M06-T13 — Map streaming failures safely

## Outcome

Define behavior before and after response headers, including terminal SSE errors, partial text policy, retry guidance, and audit outcome.
...
2. Define behavior before and after response headers, including terminal SSE errors, partial text policy, retry guidance, and audit outcome.
...
- Tests inject provider, database, validation, and cancellation failures at several stream points.
...
**Implementation prompt:** Inspect M06-T13 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Define behavior before and after response headers, including terminal SSE errors, partial text policy, retry guidance, and audit outcome.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M06-T13 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m06-t13: map streaming failures safely`.
