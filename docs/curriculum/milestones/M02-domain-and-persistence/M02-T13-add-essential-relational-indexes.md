---
id: M02-T13
title: "Add essential relational indexes"
milestone: M02
track: critical
depends_on: ["M02-T12"]
estimate: 75m
...
Use query plans to add uniqueness, foreign-key, checkpoint, availability, and ACL lookup indexes based on named access patterns.
...
2. Use query plans to add uniqueness, foreign-key, checkpoint, availability, and ACL lookup indexes based on named access patterns.
...
- EXPLAIN evidence demonstrates the intended indexes for representative queries.
...
**Implementation prompt:** Inspect M02-T13 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Use query plans to add uniqueness, foreign-key, checkpoint, availability, and ACL lookup indexes based on named access patterns.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M02-T13 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m02-t13: add essential relational indexes`.
