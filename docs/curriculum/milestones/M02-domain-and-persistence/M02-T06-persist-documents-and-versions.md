---
id: M02-T06
title: "Persist Documents and versions"
milestone: M02
track: critical
depends_on: ["M02-T05"]
estimate: 90m
topics: ["postgresql","domain-model","migrations"]
deliverables: ["document tables"]
status: planned
---
...
Create Document persistence with stable source key, current content/auth versions, fingerprints, provenance, availability, and tombstones.
...
2. Create Document persistence with stable source key, current content/auth versions, fingerprints, provenance, availability, and tombstones.
...
- Integration tests prove idempotent upsert and independent version changes.
...
**Implementation prompt:** Inspect M02-T06 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Create Document persistence with stable source key, current content/auth versions, fingerprints, provenance, availability, and tombstones.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M02-T06 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m02-t06: persist documents and versions`.
