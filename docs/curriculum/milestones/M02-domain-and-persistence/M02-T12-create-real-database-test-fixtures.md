---
id: M02-T12
title: "Create real-database test fixtures"
milestone: M02
track: critical
depends_on: ["M02-T11"]
estimate: 90m
...
Provide isolated PostgreSQL fixtures that apply migrations and reset state without mocking SQL behavior.
...
2. Provide isolated PostgreSQL fixtures that apply migrations and reset state without mocking SQL behavior.
...
- Parallel integration tests do not share rows and migration drift fails visibly.
...
**Implementation prompt:** Inspect M02-T12 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Provide isolated PostgreSQL fixtures that apply migrations and reset state without mocking SQL behavior.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M02-T12 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m02-t12: create real database test fixtures`.
