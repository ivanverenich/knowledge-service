---
id: M02-T14
title: "Pass the persistence understanding gate"
milestone: M02
track: critical
depends_on: ["M02-T13"]
estimate: 90m
topics: ["postgresql","domain-model","migrations"]
deliverables: ["M02 gate evidence"]
status: planned
---
...
Explain version separation and transaction ownership, update the data diagram, repair an injected migration failure, and add one new invariant.
...
2. Explain version separation and transaction ownership, update the data diagram, repair an injected migration failure, and add one new invariant.
...
- Evidence proves the model rather than ORM mechanics determined the repair.
...
**Implementation prompt:** Inspect M02-T14 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Explain version separation and transaction ownership, update the data diagram, repair an injected migration failure, and add one new invariant.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M02-T14 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m02-t14: pass the persistence gate`.
