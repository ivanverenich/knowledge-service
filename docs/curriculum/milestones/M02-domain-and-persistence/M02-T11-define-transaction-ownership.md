---
id: M02-T11
title: "Define transaction ownership"
milestone: M02
track: critical
depends_on: ["M02-T10"]
estimate: 75m
...
Make application workflows own transaction boundaries and document where external calls occur relative to locks and commits.
...
2. Make application workflows own transaction boundaries and document where external calls occur relative to locks and commits.
...
- A failure-injection integration test proves no half-published Document is visible.
...
**Implementation prompt:** Inspect M02-T11 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Make application workflows own transaction boundaries and document where external calls occur relative to locks and commits.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M02-T11 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m02-t11: define transaction ownership`.
