---
id: M02-T08
title: "Persist normalized Access Grants"
milestone: M02
track: critical
depends_on: ["M02-T07"]
estimate: 90m
...
Model user/group subjects and read grants with constraints that prevent malformed or ownerless permissions.
...
2. Model user/group subjects and read grants with constraints that prevent malformed or ownerless permissions.
...
- Queries can distinguish public, user, group, and no-access documents without application filtering.
...
**Implementation prompt:** Inspect M02-T08 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Model user/group subjects and read grants with constraints that prevent malformed or ownerless permissions.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M02-T08 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.
