---
id: M08-T15
title: "Build Confluence contract fixtures"
milestone: M08
track: critical
depends_on: ["M08-T14"]
estimate: 75m
topics: ["redis","dramatiq","confluence"]
deliverables: ["Confluence fixture library","live-test marker"]
status: planned
---

# M08-T15 — Build Confluence contract fixtures

## Outcome

Maintain sanitized recorded/synthetic responses and an opt-in live contract suite that cannot run without explicit credentials.
...
2. Maintain sanitized recorded/synthetic responses and an opt-in live contract suite that cannot run without explicit credentials.
...
- Offline CI detects parser/client drift; live tests are clearly marked and secret-safe.
...
**Implementation prompt:** Inspect M08-T15, its dependencies, and current evidence. Propose a bounded plan for: “Maintain sanitized recorded/synthetic responses and an opt-in live contract suite that cannot run without explicit credentials.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T15 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t15: build confluence contract fixtures`.
