---
id: M08-T05
title: "Coordinate with expiring locks"
milestone: M08
track: critical
depends_on: ["M08-T04"]
estimate: 75m
topics: ["redis","dramatiq","confluence"]
deliverables: ["coordination-lock module"]
status: planned
---

# M08-T05 — Coordinate with expiring locks

## Outcome

Use short-lived Redis locks only where duplicate concurrent work is unsafe, with ownership tokens and database truth after expiry.
...
2. Use short-lived Redis locks only where duplicate concurrent work is unsafe, with ownership tokens and database truth after expiry.
...
- Crash and expiry tests avoid permanent lockout and do not claim locks guarantee correctness.
...
**Implementation prompt:** Inspect M08-T05, its dependencies, and current evidence. Propose a bounded plan for: “Use short-lived Redis locks only where duplicate concurrent work is unsafe, with ownership tokens and database truth after expiry.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T05 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t05: coordinate with expiring locks`.
