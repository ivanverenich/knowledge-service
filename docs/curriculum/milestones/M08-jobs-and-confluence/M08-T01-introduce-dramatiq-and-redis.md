---
id: M08-T01
title: "Introduce Dramatiq and Redis"
milestone: M08
track: critical
depends_on: ["M07-T16"]
estimate: 90m
topics: ["redis","dramatiq","confluence"]
deliverables: ["worker runtime","Redis broker configuration"]
status: planned
---

# M08-T01 — Introduce Dramatiq and Redis

## Outcome

Configure versioned Dramatiq/Redis infrastructure with explicit queues, serializers, time limits, and local health checks.
...
2. Configure versioned Dramatiq/Redis infrastructure with explicit queues, serializers, time limits, and local health checks.
...
- A worker consumes a harmless job and fails readiness when its broker is unavailable.
...
**Implementation prompt:** Inspect M08-T01, its dependencies, and current evidence. Propose a bounded plan for: “Configure versioned Dramatiq/Redis infrastructure with explicit queues, serializers, time limits, and local health checks.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T01 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t01: introduce dramatiq and redis`.
