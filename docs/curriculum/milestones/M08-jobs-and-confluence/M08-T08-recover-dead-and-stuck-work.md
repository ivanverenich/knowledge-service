---
id: M08-T08
title: "Recover dead and stuck work"
milestone: M08
track: critical
depends_on: ["M08-T07"]
estimate: 90m
topics: ["redis","dramatiq","confluence"]
deliverables: ["job recovery workflow"]
status: planned
---

# M08-T08 — Recover dead and stuck work

## Outcome

Detect exhausted, stale, and orphaned Jobs, expose operator decisions, and support safe retry or cancellation.
...
2. Detect exhausted, stale, and orphaned Jobs, expose operator decisions, and support safe retry or cancellation.
...
- A recovery drill distinguishes running-slow from dead and preserves prior attempt evidence.
...
**Implementation prompt:** Inspect M08-T08, its dependencies, and current evidence. Propose a bounded plan for: “Detect exhausted, stale, and orphaned Jobs, expose operator decisions, and support safe retry or cancellation.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T08 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t08: recover dead and stuck work`.
