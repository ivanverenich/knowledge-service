---
id: M08-T04
title: "Assign retry and timeout ownership"
milestone: M08
track: critical
depends_on: ["M08-T03"]
estimate: 75m
topics: ["redis","dramatiq","confluence"]
deliverables: ["retry policy"]
status: planned
---

# M08-T04 — Assign retry and timeout ownership

## Outcome

Classify source, database, model, and programming failures; place bounded retry at one layer with backoff and jitter.
...
2. Classify source, database, model, and programming failures; place bounded retry at one layer with backoff and jitter.
...
- Tests prove permanent errors do not retry and temporary errors stop at the configured budget.
...
**Implementation prompt:** Inspect M08-T04, its dependencies, and current evidence. Propose a bounded plan for: “Classify source, database, model, and programming failures; place bounded retry at one layer with backoff and jitter.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T04 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t04: assign retry and timeout ownership`.
