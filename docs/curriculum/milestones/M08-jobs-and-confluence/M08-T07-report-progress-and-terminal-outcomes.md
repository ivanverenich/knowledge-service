---
id: M08-T07
title: "Report progress and terminal outcomes"
milestone: M08
track: critical
depends_on: ["M08-T06"]
estimate: 75m
topics: ["redis","dramatiq","confluence"]
deliverables: ["job progress model"]
status: planned
---

# M08-T07 — Report progress and terminal outcomes

## Outcome

Persist monotonic counters, stage, heartbeat, warnings, and typed terminal outcome without coupling the domain to Dramatiq messages.
...
2. Persist monotonic counters, stage, heartbeat, warnings, and typed terminal outcome without coupling the domain to Dramatiq messages.
...
- API-visible progress survives worker restart and rejects backward transitions.
...
**Implementation prompt:** Inspect M08-T07, its dependencies, and current evidence. Propose a bounded plan for: “Persist monotonic counters, stage, heartbeat, warnings, and typed terminal outcome without coupling the domain to Dramatiq messages.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T07 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.
