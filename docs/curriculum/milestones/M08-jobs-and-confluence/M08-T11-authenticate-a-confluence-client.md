---
id: M08-T11
title: "Authenticate a Confluence client"
milestone: M08
track: critical
depends_on: ["M08-T10"]
estimate: 90m
topics: ["redis","dramatiq","confluence"]
deliverables: ["Confluence HTTP client"]
status: planned
---

# M08-T11 — Authenticate a Confluence client

## Outcome

Create an async Cloud REST client with secret references, base URL validation, scopes documentation, deadlines, and safe request metadata.
...
2. Create an async Cloud REST client with secret references, base URL validation, scopes documentation, deadlines, and safe request metadata.
...
- HTTP contract tests cover auth failure, rate limits, timeout, invalid host, and redacted diagnostics.
...
**Implementation prompt:** Inspect M08-T11, its dependencies, and current evidence. Propose a bounded plan for: “Create an async Cloud REST client with secret references, base URL validation, scopes documentation, deadlines, and safe request metadata.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T11 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t11: authenticate a confluence client`.
