---
id: M08-T13
title: "Normalize Confluence bodies"
milestone: M08
track: critical
depends_on: ["M08-T12"]
estimate: 90m
topics: ["redis","dramatiq","confluence"]
deliverables: ["Confluence content mapper"]
status: planned
---

# M08-T13 — Normalize Confluence bodies

## Outcome

Translate selected body representation, headings, links, macros, and tables into Source Records while quarantining unsupported content.
...
2. Translate selected body representation, headings, links, macros, and tables into Source Records while quarantining unsupported content.
...
- Fixtures cover common macros, malformed storage HTML, attachments metadata, and provenance URLs.
...
**Implementation prompt:** Inspect M08-T13, its dependencies, and current evidence. Propose a bounded plan for: “Translate selected body representation, headings, links, macros, and tables into Source Records while quarantining unsupported content.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T13 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t13: normalize confluence bodies`.
