---
id: M08-T14
title: "Map Confluence permissions"
milestone: M08
track: critical
depends_on: ["M08-T13"]
estimate: 90m
topics: ["redis","dramatiq","confluence"]
deliverables: ["Confluence grant mapper"]
status: planned
---

# M08-T14 — Map Confluence permissions

## Outcome

Combine site, space, and content restrictions into normalized Access Grants with fail-closed unresolved users/groups.
...
2. Combine site, space, and content restrictions into normalized Access Grants with fail-closed unresolved users/groups.
...
- Fixture tests include inherited access, restricted pages, removed groups, and permission-only changes.
...
**Implementation prompt:** Inspect M08-T14, its dependencies, and current evidence. Propose a bounded plan for: “Combine site, space, and content restrictions into normalized Access Grants with fail-closed unresolved users/groups.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T14 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t14: map confluence permissions`.
