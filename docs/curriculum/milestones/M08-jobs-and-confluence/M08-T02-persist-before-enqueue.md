...
deliverables: ["job submission workflow"]
status: planned
---

# M08-T02 — Persist before enqueue

## Outcome

Create durable Job identity and intent in PostgreSQL before publishing a message, with recovery for persisted-but-unpublished work.
...
2. Create durable Job identity and intent in PostgreSQL before publishing a message, with recovery for persisted-but-unpublished work.
...
- Failure injection between commit and publish is detected and repaired without duplicate effects.
...
**Implementation prompt:** Inspect M08-T02, its dependencies, and current evidence. Propose a bounded plan for: “Create durable Job identity and intent in PostgreSQL before publishing a message, with recovery for persisted-but-unpublished work.” Wait for approval, then make only task-scoped changes and report exact checks and external spend.

**Review prompt:** Review M08-T02 for contract drift, authorization, idempotency, async cleanup, observability, test determinism, and hidden paid/network dependency. Lead with reproducible findings.

## Git checkpoint

Commit only with complete evidence, using intent: `m08-t02: persist before enqueue`.
