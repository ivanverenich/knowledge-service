---
id: M00-T08
title: "Model typed configuration"
milestone: M00
track: critical
depends_on: ["M00-T07"]
estimate: 90m
...
Introduce Pydantic Settings with local-safe defaults, explicit secret fields, environment validation, and a documented example environment file.
...
2. Introduce Pydantic Settings with local-safe defaults, explicit secret fields, environment validation, and a documented example environment file.
...
- Tests prove missing required settings fail clearly and secret representations are redacted.
...
**Implementation prompt:** Inspect M00-T08 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Introduce Pydantic Settings with local-safe defaults, explicit secret fields, environment validation, and a documented example environment file.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T08 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.
