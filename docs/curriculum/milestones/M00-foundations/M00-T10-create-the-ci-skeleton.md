...
Add GitHub Actions jobs for style, types, and deterministic tests with uv caching, cancellation, least permissions, and explicit timeouts.
...
2. Add GitHub Actions jobs for style, types, and deterministic tests with uv caching, cancellation, least permissions, and explicit timeouts.
...
- The workflow syntax validates and local commands match the commands CI will execute.
...
**Implementation prompt:** Inspect M00-T10 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Add GitHub Actions jobs for style, types, and deterministic tests with uv caching, cancellation, least permissions, and explicit timeouts.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T10 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m00-t10: create the ci skeleton`.
