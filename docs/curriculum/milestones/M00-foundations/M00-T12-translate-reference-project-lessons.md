---
id: M00-T12
title: "Translate the reference-project lessons"
milestone: M00
track: critical
depends_on: ["M00-T11"]
estimate: 60m
...
Inspect the named browser-use files and write a short comparison mapping borrowed practices to this smaller system and recording rejected complexity.
...
2. Inspect the named browser-use files and write a short comparison mapping borrowed practices to this smaller system and recording rejected complexity.
...
- Every claimed borrowed practice points to a concrete reference path and a local reason.
...
**Implementation prompt:** Inspect M00-T12 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Inspect the named browser-use files and write a short comparison mapping borrowed practices to this smaller system and recording rejected complexity.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T12 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m00-t12: translate reference project lessons`.
