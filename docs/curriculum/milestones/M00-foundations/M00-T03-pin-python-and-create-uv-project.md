---
id: M00-T03
title: "Pin Python 3.14 and create the uv project"
milestone: M00
track: critical
depends_on: ["M00-T02"]
estimate: 75m
...
Create project metadata with Python 3.14, a package build backend, dependency groups, and a committed universal lockfile.
...
2. Create project metadata with Python 3.14, a package build backend, dependency groups, and a committed universal lockfile.
...
- A fresh uv environment reports Python 3.14 and installs from the lockfile reproducibly.
...
**Implementation prompt:** Inspect M00-T03 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Create project metadata with Python 3.14, a package build backend, dependency groups, and a committed universal lockfile.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T03 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.
