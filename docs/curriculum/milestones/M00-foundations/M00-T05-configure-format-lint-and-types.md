---
id: M00-T05
title: "Configure formatting, linting, and type checking"
milestone: M00
track: critical
depends_on: ["M00-T04"]
estimate: 75m
...
Configure Ruff and a strict type checker for Python 3.14, including async correctness rules and deliberate exclusions.
...
2. Configure Ruff and a strict type checker for Python 3.14, including async correctness rules and deliberate exclusions.
...
- One command formats/checks the seed package and fails on a planted lint and type violation.
...
**Implementation prompt:** Inspect M00-T05 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Configure Ruff and a strict type checker for Python 3.14, including async correctness rules and deliberate exclusions.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T05 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.
