---
id: M00-T04
title: "Choose the initial package layout"
milestone: M00
track: critical
depends_on: ["M00-T03"]
estimate: 60m
...
Create the smallest src-layout package and test tree needed for an importable knowledge service; document why feature modules are added only when behavior exists.
...
2. Create the smallest src-layout package and test tree needed for an importable knowledge service; document why feature modules are added only when behavior exists.
...
- The package imports from an isolated environment and contains no speculative domain folders.
...
**Implementation prompt:** Inspect M00-T04 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Create the smallest src-layout package and test tree needed for an importable knowledge service; document why feature modules are added only when behavior exists.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T04 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.
