---
id: M00-T14
title: "Pass the foundations understanding gate"
milestone: M00
track: critical
depends_on: ["M00-T13"]
estimate: 90m
...
Explain the toolchain and package layout, redraw the development loop, diagnose an injected configuration failure, and add one small check without procedural instructions.
...
2. Explain the toolchain and package layout, redraw the development loop, diagnose an injected configuration failure, and add one small check without procedural instructions.
...
- The written explanation, updated diagram, diagnosis, and unscripted change all have reviewable evidence.
...
**Implementation prompt:** Inspect M00-T14 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Explain the toolchain and package layout, redraw the development loop, diagnose an injected configuration failure, and add one small check without procedural instructions.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T14 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m00-t14: pass the foundations gate`.
