---
id: M00-T01
title: "Record the workspace baseline"
milestone: M00
track: critical
depends_on: []
estimate: 45m
topics: ["python","tooling","testing"]
deliverables: ["environment baseline"]
status: completed
---

# M00-T01 — Record the workspace baseline

## Outcome

Record OS, CPU architecture, installed tools, and the initially empty workspace without installing or generating application files.
...
2. Record OS, CPU architecture, installed tools, and the initially empty workspace without installing or generating application files.
...
- A checked-in baseline note distinguishes prerequisites from changes made by the curriculum.
...
**Implementation prompt:** Inspect M00-T01 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Record OS, CPU architecture, installed tools, and the initially empty workspace without installing or generating application files.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M00-T01 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m00-t01: record the workspace baseline`.
