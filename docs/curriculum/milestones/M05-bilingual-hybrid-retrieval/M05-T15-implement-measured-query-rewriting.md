---
id: M05-T15
title: "Implement measured query rewriting"
milestone: M05
track: critical
depends_on: ["M05-T14"]
estimate: 90m
topics: ["hybrid-search","ukrainian","reranking"]
deliverables: ["query-rewrite module","rewrite evaluation"]
status: planned
---

# M05-T15 — Implement measured query rewriting

## Outcome

Add structured rewriting behind a policy flag, preserve original terms/language, and fall back safely when validation or deadlines fail.
...
2. Add structured rewriting behind a policy flag, preserve original terms/language, and fall back safely when validation or deadlines fail.
...
- Evaluation reports per-class gain and prevents rollout where rewriting harms retrieval.
...
**Implementation prompt:** Inspect M05-T15 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Add structured rewriting behind a policy flag, preserve original terms/language, and fall back safely when validation or deadlines fail.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M05-T15 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m05-t15: implement measured query rewriting`.
