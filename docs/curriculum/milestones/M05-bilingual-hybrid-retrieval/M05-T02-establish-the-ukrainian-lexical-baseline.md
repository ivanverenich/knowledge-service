---
id: M05-T02
title: "Establish the Ukrainian lexical baseline"
milestone: M05
track: critical
depends_on: ["M05-T01"]
estimate: 90m
topics: ["hybrid-search","ukrainian","reranking"]
deliverables: ["Ukrainian FTS baseline","limitation report"]
status: planned
---

# M05-T02 — Establish the Ukrainian lexical baseline

## Outcome

Index Ukrainian text with PostgreSQL simple configuration, documenting absent stemming and expected token-matching limits.
...
2. Index Ukrainian text with PostgreSQL simple configuration, documenting absent stemming and expected token-matching limits.
...
- A Ukrainian fixture report shows successful exact matches and known morphological misses.
...
**Implementation prompt:** Inspect M05-T02 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Index Ukrainian text with PostgreSQL simple configuration, documenting absent stemming and expected token-matching limits.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M05-T02 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m05-t02: establish the ukrainian lexical baseline`.
