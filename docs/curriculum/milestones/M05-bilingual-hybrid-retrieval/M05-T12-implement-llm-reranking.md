---
id: M05-T12
title: "Implement LLM reranking"
milestone: M05
track: critical
depends_on: ["M05-T11"]
estimate: 90m
topics: ["hybrid-search","ukrainian","reranking"]
deliverables: ["LLM reranker adapter"]
status: planned
---

# M05-T12 — Implement LLM reranking

## Outcome

Request structured rankings over opaque Candidate IDs with a strict candidate/token budget and validate duplicates, omissions, and unknown IDs.
...
2. Request structured rankings over opaque Candidate IDs with a strict candidate/token budget and validate duplicates, omissions, and unknown IDs.
...
- Tests cover malformed rankings, timeout fallback, prompt injection text, and cost capture.
...
**Implementation prompt:** Inspect M05-T12 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Request structured rankings over opaque Candidate IDs with a strict candidate/token budget and validate duplicates, omissions, and unknown IDs.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M05-T12 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m05-t12: implement llm reranking`.
