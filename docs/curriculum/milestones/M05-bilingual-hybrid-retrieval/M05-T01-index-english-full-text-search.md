---
id: M05-T01
title: "Index English full-text search"
milestone: M05
track: critical
depends_on: ["M04-T12"]
estimate: 90m
topics: ["hybrid-search","ukrainian","reranking"]
deliverables: ["English FTS migration","English lexical query"]
status: planned
---

# M05-T01 — Index English full-text search

## Outcome

Create weighted English tsvectors from title, headings, and body with a GIN index and explicit configuration.
...
2. Create weighted English tsvectors from title, headings, and body with a GIN index and explicit configuration.
...
- Queries demonstrate stemming, weighting, exact terms, and safe web-search syntax.
...
**Implementation prompt:** Inspect M05-T01 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Create weighted English tsvectors from title, headings, and body with a GIN index and explicit configuration.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M05-T01 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m05-t01: index english full text search`.
