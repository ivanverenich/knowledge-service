---
id: M05-T08
title: "Measure managed multilingual embeddings"
milestone: M05
track: critical
depends_on: ["M05-T07"]
estimate: 90m
topics: ["hybrid-search","ukrainian","reranking"]
deliverables: ["managed embedding report"]
status: planned
---

# M05-T08 — Measure managed multilingual embeddings

## Outcome

Run the seed dataset through the managed embeddings path and slice Recall@k and MRR by language direction.
...
2. Run the seed dataset through the managed embeddings path and slice Recall@k and MRR by language direction.
...
- A versioned report includes configuration, misses, and at least one falsified assumption.
...
**Implementation prompt:** Inspect M05-T08 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Run the seed dataset through the managed embeddings path and slice Recall@k and MRR by language direction.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M05-T08 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m05-t08: measure managed multilingual embeddings`.
