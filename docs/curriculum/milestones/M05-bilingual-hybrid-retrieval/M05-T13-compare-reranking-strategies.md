---
id: M05-T13
title: "Compare reranking strategies"
milestone: M05
track: critical
depends_on: ["M05-T12"]
estimate: 90m
topics: ["hybrid-search","ukrainian","reranking"]
deliverables: ["reranking comparison"]
status: planned
---

# M05-T13 — Compare reranking strategies

## Outcome

Evaluate no reranker, local cross-encoder, and LLM reranker on the same bilingual cases and report quality-latency-cost tradeoffs.
...
2. Evaluate no reranker, local cross-encoder, and LLM reranker on the same bilingual cases and report quality-latency-cost tradeoffs.
...
- A decision selects defaults per environment or rejects reranking when gain is insufficient.
...
**Implementation prompt:** Inspect M05-T13 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Evaluate no reranker, local cross-encoder, and LLM reranker on the same bilingual cases and report quality-latency-cost tradeoffs.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M05-T13 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m05-t13: compare reranking strategies`.
