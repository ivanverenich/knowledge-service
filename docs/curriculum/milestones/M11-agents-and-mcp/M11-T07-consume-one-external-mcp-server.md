---
id: M11-T07
title: "Consume one external MCP server"
milestone: M11
track: optional
depends_on: ["M11-T06"]
estimate: 90m
topics: ["langgraph","mcp","multi-agent"]
deliverables: ["MCP client adapter"]
status: planned
---

# M11-T07 — Consume one external MCP server

## Outcome

Add a controlled read-only external tool server with allowlisted capabilities, result validation, deadlines, provenance, and disabled-by-default configuration.
...
2. Add a controlled read-only external tool server with allowlisted capabilities, result validation, deadlines, provenance, and disabled-by-default configuration.
...
- Malicious, oversized, unavailable, and schema-invalid tool results fail safely.
...
**Implementation prompt:** Inspect M11-T07, prerequisites, and current interfaces. Propose a bounded plan for: “Add a controlled read-only external tool server with allowlisted capabilities, result validation, deadlines, provenance, and disabled-by-default configuration.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M11-T07 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m11-t07: consume one external mcp server`.
