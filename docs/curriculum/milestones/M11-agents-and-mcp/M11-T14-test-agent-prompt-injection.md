---
id: M11-T14
title: "Test agent prompt injection"
milestone: M11
track: optional
depends_on: ["M11-T13"]
estimate: 90m
topics: ["langgraph","mcp","multi-agent"]
deliverables: ["agent adversarial suite"]
status: planned
---

# M11-T14 — Test agent prompt injection

## Outcome

Attack tool descriptions, retrieved documents, external results, encoded instructions, and fake authorization claims.
...
2. Attack tool descriptions, retrieved documents, external results, encoded instructions, and fake authorization claims.
...
- The adversarial suite shows tools remain read-only, identity is server-owned, and leaked content stays zero.
...
**Implementation prompt:** Inspect M11-T14, prerequisites, and current interfaces. Propose a bounded plan for: “Attack tool descriptions, retrieved documents, external results, encoded instructions, and fake authorization claims.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M11-T14 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m11-t14: test agent prompt injection`.
