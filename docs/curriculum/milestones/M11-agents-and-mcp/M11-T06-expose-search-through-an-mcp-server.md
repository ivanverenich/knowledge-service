---
id: M11-T06
title: "Expose search through an MCP server"
milestone: M11
track: optional
depends_on: ["M11-T05"]
estimate: 90m
...
# M11-T06 — Expose search through an MCP server

## Outcome

Map knowledge tools into an MCP server with schemas, capability discovery, transport limits, identity propagation, and structured errors.
...
2. Map knowledge tools into an MCP server with schemas, capability discovery, transport limits, identity propagation, and structured errors.
...
- An MCP client contract test discovers and invokes tools without bypassing authorization.
...
**Implementation prompt:** Inspect M11-T06, prerequisites, and current interfaces. Propose a bounded plan for: “Map knowledge tools into an MCP server with schemas, capability discovery, transport limits, identity propagation, and structured errors.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M11-T06 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m11-t06: expose search through an mcp server`.
