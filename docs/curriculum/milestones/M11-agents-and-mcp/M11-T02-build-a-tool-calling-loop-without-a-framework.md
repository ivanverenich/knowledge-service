---
id: M11-T02
title: "Build a tool-calling loop without a framework"
milestone: M11
track: optional
depends_on: ["M11-T01"]
estimate: 90m
topics: ["langgraph","mcp","multi-agent"]
deliverables: ["bounded tool loop"]
status: planned
---

# M11-T02 — Build a tool-calling loop without a framework

## Outcome

Implement a bounded model/tool loop with structured calls, maximum steps, deadline, token/cost budget, duplicate-call detection, and terminal outcomes.
...
2. Implement a bounded model/tool loop with structured calls, maximum steps, deadline, token/cost budget, duplicate-call detection, and terminal outcomes.
...
- Deterministic scripted-model tests cover success, bad arguments, repeated calls, limit exhaustion, and cancellation.
...
**Implementation prompt:** Inspect M11-T02, prerequisites, and current interfaces. Propose a bounded plan for: “Implement a bounded model/tool loop with structured calls, maximum steps, deadline, token/cost budget, duplicate-call detection, and terminal outcomes.” Wait for approval; then report changes, tests/drills, spend, and residual risk.

**Review prompt:** Review M11-T02 for interface leakage, identity/budget propagation, failure recovery, reproducibility, supply-chain risk, and misleading success claims. Lead with evidence-backed findings.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m11-t02: build a tool calling loop without a framework`.
