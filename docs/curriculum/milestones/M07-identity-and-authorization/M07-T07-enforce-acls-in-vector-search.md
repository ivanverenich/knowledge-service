---
id: M07-T07
title: "Enforce ACLs in vector search"
milestone: M07
track: critical
depends_on: ["M07-T06"]
estimate: 90m
topics: ["oidc","authorization","security"]
deliverables: ["authorized vector query"]
status: planned
---

# M07-T07 — Enforce ACLs in vector search

## Outcome

Join current Documents and Access Grants inside the vector Candidate query using the Authorization Context and availability state.
...
2. Join current Documents and Access Grants inside the vector Candidate query using the Authorization Context and availability state.
...
- Adversarial data proves unauthorized high-similarity Chunks are never returned.
...
**Implementation prompt:** Inspect M07-T07 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Join current Documents and Access Grants inside the vector Candidate query using the Authorization Context and availability state.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T07 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t07: enforce acls in vector search`.
