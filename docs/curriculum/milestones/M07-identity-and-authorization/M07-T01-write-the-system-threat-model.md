---
id: M07-T01
title: "Write the system threat model"
milestone: M07
track: critical
depends_on: ["M06-T14"]
estimate: 90m
...
# M07-T01 — Write the system threat model

## Outcome

Identify assets, actors, trust zones, entry points, abuse cases, and mitigations for ingestion, retrieval, generation, agents, and operations.
...
2. Identify assets, actors, trust zones, entry points, abuse cases, and mitigations for ingestion, retrieval, generation, agents, and operations.
...
- The model traces every security invariant in the security document to a control and test owner.
...
**Implementation prompt:** Inspect M07-T01 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Identify assets, actors, trust zones, entry points, abuse cases, and mitigations for ingestion, retrieval, generation, agents, and operations.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T01 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t01: write the system threat model`.
