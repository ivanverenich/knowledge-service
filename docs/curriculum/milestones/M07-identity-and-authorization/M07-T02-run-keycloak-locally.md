...
deliverables: ["Keycloak local configuration"]
status: planned
---

# M07-T02 — Run Keycloak locally

## Outcome

Add a pinned local Keycloak configuration with a realm, clients, redirect URIs, signing keys, and non-production demo users/groups.
...
2. Add a pinned local Keycloak configuration with a realm, clients, redirect URIs, signing keys, and non-production demo users/groups.
...
- A clean Docker volume reproduces login and token issuance without committed credentials.
...
**Implementation prompt:** Inspect M07-T02 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Add a pinned local Keycloak configuration with a realm, clients, redirect URIs, signing keys, and non-production demo users/groups.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T02 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t02: run keycloak locally`.
