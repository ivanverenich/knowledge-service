---
id: M07-T08
title: "Enforce ACLs in lexical search"
milestone: M07
track: critical
depends_on: ["M07-T07"]
estimate: 90m
topics: ["oidc","authorization","security"]
deliverables: ["authorized lexical query"]
status: planned
---

# M07-T08 — Enforce ACLs in lexical search

## Outcome

Apply the identical permission semantics to English and Ukrainian lexical branches before ranking or highlighting.
...
2. Apply the identical permission semantics to English and Ukrainian lexical branches before ranking or highlighting.
...
- Shared security contract tests run against dense and lexical search.
...
**Implementation prompt:** Inspect M07-T08 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Apply the identical permission semantics to English and Ukrainian lexical branches before ranking or highlighting.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T08 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t08: enforce acls in lexical search`.
