---
id: M07-T13
title: "Harden against retrieved instructions"
milestone: M07
track: critical
depends_on: ["M07-T12"]
estimate: 90m
topics: ["oidc","authorization","security"]
deliverables: ["prompt-injection controls"]
status: planned
---

# M07-T13 — Harden against retrieved instructions

## Outcome

Delimit untrusted Evidence, keep tools policy-controlled, validate outputs, and seed prompt-injection documents in tests.
...
2. Delimit untrusted Evidence, keep tools policy-controlled, validate outputs, and seed prompt-injection documents in tests.
...
- Adversarial cases fail to alter tool access, reveal hidden content, or create unvalidated citations.
...
**Implementation prompt:** Inspect M07-T13 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Delimit untrusted Evidence, keep tools policy-controlled, validate outputs, and seed prompt-injection documents in tests.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T13 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t13: harden against retrieved instructions`.
