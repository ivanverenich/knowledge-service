---
id: M07-T09
title: "Authorize Document fetches"
milestone: M07
track: critical
depends_on: ["M07-T08"]
estimate: 90m
topics: ["oidc","authorization","security"]
deliverables: ["authorized document reader"]
status: planned
---

# M07-T09 — Authorize Document fetches

## Outcome

Protect full-document, excerpt, citation-resolution, export, and agent-tool reads with the same current Access Grants.
...
2. Protect full-document, excerpt, citation-resolution, export, and agent-tool reads with the same current Access Grants.
...
- Tests attempt direct-object-reference attacks using valid IDs from another User.
...
**Implementation prompt:** Inspect M07-T09 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Protect full-document, excerpt, citation-resolution, export, and agent-tool reads with the same current Access Grants.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T09 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t09: authorize document fetches`.
