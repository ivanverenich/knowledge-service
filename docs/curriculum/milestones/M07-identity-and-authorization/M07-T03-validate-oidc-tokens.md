...
deliverables: ["OIDC verifier"]
status: planned
---

# M07-T03 — Validate OIDC tokens

## Outcome

Verify issuer, audience, signature, algorithm, expiry, and clock skew using cached discovery/JWKS with bounded refresh failure behavior.
...
2. Verify issuer, audience, signature, algorithm, expiry, and clock skew using cached discovery/JWKS with bounded refresh failure behavior.
...
- Tests cover valid, expired, wrong-audience, wrong-issuer, unknown-key, and key-rotation tokens.
...
**Implementation prompt:** Inspect M07-T03 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Verify issuer, audience, signature, algorithm, expiry, and clock skew using cached discovery/JWKS with bounded refresh failure behavior.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T03 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t03: validate oidc tokens`.
