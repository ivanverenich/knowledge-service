---
id: M07-T11
title: "Eliminate existence leaks"
milestone: M07
track: critical
depends_on: ["M07-T10"]
estimate: 75m
topics: ["oidc","authorization","security"]
deliverables: ["existence-leak policy"]
status: planned
---

# M07-T11 — Eliminate existence leaks

## Outcome

Make unauthorized and absent Documents indistinguishable where appropriate across errors, counts, timing-sensitive paths, and model context.
...
2. Make unauthorized and absent Documents indistinguishable where appropriate across errors, counts, timing-sensitive paths, and model context.
...
- Black-box tests compare responses for missing and forbidden identifiers.
...
**Implementation prompt:** Inspect M07-T11 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Make unauthorized and absent Documents indistinguishable where appropriate across errors, counts, timing-sensitive paths, and model context.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T11 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t11: eliminate existence leaks`.
