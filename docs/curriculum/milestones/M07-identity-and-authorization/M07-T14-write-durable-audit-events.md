---
id: M07-T14
title: "Write durable audit events"
milestone: M07
track: critical
depends_on: ["M07-T13"]
estimate: 90m
topics: ["oidc","authorization","security"]
deliverables: ["audit writer"]
status: planned
---

# M07-T14 — Write durable audit events

## Outcome

Record actor, action, object identifiers, authorization decision, outcome, request/run IDs, and time without raw sensitive content.
...
2. Record actor, action, object identifiers, authorization decision, outcome, request/run IDs, and time without raw sensitive content.
...
- Integration tests cover query, document read, grant change, synchronization, feedback, and administration.
...
**Implementation prompt:** Inspect M07-T14 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Record actor, action, object identifiers, authorization decision, outcome, request/run IDs, and time without raw sensitive content.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T14 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t14: write durable audit events`.
