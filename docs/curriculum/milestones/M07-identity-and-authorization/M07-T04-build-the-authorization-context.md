---
id: M07-T04
title: "Build the Authorization Context"
milestone: M07
track: critical
depends_on: ["M07-T03"]
estimate: 75m
topics: ["oidc","authorization","security"]
deliverables: ["Authorization Context builder"]
status: planned
---

# M07-T04 — Build the Authorization Context

## Outcome

Map verified subject and group claims into typed internal identity, rejecting oversized, malformed, or untrusted client-supplied claims.
...
2. Map verified subject and group claims into typed internal identity, rejecting oversized, malformed, or untrusted client-supplied claims.
...
- Endpoint tests prove only verifier output can create an Authorization Context.
...
**Implementation prompt:** Inspect M07-T04 and prerequisite evidence. Describe the existing seam and propose a scoped plan for: “Map verified subject and group claims into typed internal identity, rejecting oversized, malformed, or untrusted client-supplied claims.” Wait for approval before edits; report changed files, tests, and unresolved risks.

**Review prompt:** Review M07-T04 for behavior, security/ACL ordering, async resource ownership, deterministic coverage, and accidental scope. Lead with findings tied to observable failures.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m07-t04: build the authorization context`.
