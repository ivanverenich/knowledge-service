---
id: M03-T09
title: "Implement fixed-window chunking"
milestone: M03
track: critical
depends_on: ["M03-T08"]
estimate: 90m
topics: ["ingestion","parsing","chunking"]
deliverables: ["fixed-window chunker"]
status: planned
---

# M03-T09 — Implement fixed-window chunking

## Outcome

Create the simple token-window baseline with bounded overlap, stable ordering, token counts, and reversible provenance spans.
...
2. Create the simple token-window baseline with bounded overlap, stable ordering, token counts, and reversible provenance spans.
...
- Tests cover small, exact-boundary, oversized, Unicode, and empty Documents.
...
**Implementation prompt:** Inspect M03-T09 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Create the simple token-window baseline with bounded overlap, stable ordering, token counts, and reversible provenance spans.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T09 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t09: implement fixed window chunking`.
