---
id: M03-T06
title: "Normalize Source Records into Documents"
milestone: M03
track: critical
depends_on: ["M03-T05"]
estimate: 90m
...
# M03-T06 — Normalize Source Records into Documents

## Outcome

Build a normalization module that selects a parser, canonicalizes metadata, records parser version, detects empty content, and returns typed outcomes.
...
2. Build a normalization module that selects a parser, canonicalizes metadata, records parser version, detects empty content, and returns typed outcomes.
...
- The same Source Record produces a deterministic Document and typed quarantine result on failure.
...
**Implementation prompt:** Inspect M03-T06 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Build a normalization module that selects a parser, canonicalizes metadata, records parser version, detects empty content, and returns typed outcomes.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T06 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t06: normalize source records`.
