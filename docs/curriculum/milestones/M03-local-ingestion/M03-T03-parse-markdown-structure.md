---
id: M03-T03
title: "Parse Markdown structure"
milestone: M03
track: critical
depends_on: ["M03-T02"]
estimate: 90m
topics: ["ingestion","parsing","chunking"]
deliverables: ["Markdown parser","golden fixtures"]
status: planned
---

# M03-T03 — Parse Markdown structure

## Outcome

Extract headings, paragraphs, lists, code blocks, links, and tables into normalized structural blocks while preserving source offsets.
...
2. Extract headings, paragraphs, lists, code blocks, links, and tables into normalized structural blocks while preserving source offsets.
...
- Golden tests compare normalized blocks for representative Markdown and malformed input.
...
**Implementation prompt:** Inspect M03-T03 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Extract headings, paragraphs, lists, code blocks, links, and tables into normalized structural blocks while preserving source offsets.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T03 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t03: parse markdown structure`.
