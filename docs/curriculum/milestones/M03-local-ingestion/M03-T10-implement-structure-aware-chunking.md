---
id: M03-T10
title: "Implement structure-aware chunking"
milestone: M03
track: critical
depends_on: ["M03-T09"]
estimate: 90m
topics: ["ingestion","parsing","chunking"]
deliverables: ["structure-aware chunker"]
status: planned
---

# M03-T10 — Implement structure-aware chunking

## Outcome

Prefer headings and block boundaries while respecting min/max token budgets and attaching hierarchical titles to Chunks.
...
2. Prefer headings and block boundaries while respecting min/max token budgets and attaching hierarchical titles to Chunks.
...
- Golden tests demonstrate preserved sections and defined fallback to fixed windows.
...
**Implementation prompt:** Inspect M03-T10 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Prefer headings and block boundaries while respecting min/max token budgets and attaching hierarchical titles to Chunks.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T10 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t10: implement structure aware chunking`.
