---
id: M03-T08
title: "Fingerprint content and authorization"
milestone: M03
track: critical
depends_on: ["M03-T07"]
estimate: 75m
topics: ["ingestion","parsing","chunking"]
deliverables: ["fingerprint functions"]
status: planned
---

# M03-T08 — Fingerprint content and authorization

## Outcome

Canonicalize and hash content separately from normalized Access Grants so unchanged, content-only, and ACL-only updates are distinguishable.
...
2. Canonicalize and hash content separately from normalized Access Grants so unchanged, content-only, and ACL-only updates are distinguishable.
...
- Property tests prove ordering noise in grants does not change authorization fingerprints.
...
**Implementation prompt:** Inspect M03-T08 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Canonicalize and hash content separately from normalized Access Grants so unchanged, content-only, and ACL-only updates are distinguishable.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T08 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t08: fingerprint content and authorization`.
