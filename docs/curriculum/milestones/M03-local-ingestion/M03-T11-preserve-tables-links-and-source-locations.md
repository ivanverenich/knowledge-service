---
id: M03-T11
title: "Preserve tables, links, and source locations"
milestone: M03
track: critical
depends_on: ["M03-T10"]
estimate: 75m
topics: ["ingestion","parsing","chunking"]
deliverables: ["chunk provenance mapping"]
status: planned
---

# M03-T11 — Preserve tables, links, and source locations

## Outcome

Carry normalized table text, link targets, page/heading/offset metadata, and display-safe source URLs into each Chunk.
...
2. Carry normalized table text, link targets, page/heading/offset metadata, and display-safe source URLs into each Chunk.
...
- Citation fixture assertions can reconstruct a useful excerpt and source location.
...
**Implementation prompt:** Inspect M03-T11 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Carry normalized table text, link targets, page/heading/offset metadata, and display-safe source URLs into each Chunk.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T11 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t11: preserve tables links and source locations`.
