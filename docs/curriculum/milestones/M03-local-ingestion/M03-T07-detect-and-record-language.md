---
id: M03-T07
title: "Detect and record language"
milestone: M03
track: critical
depends_on: ["M03-T06"]
estimate: 75m
topics: ["ingestion","parsing","chunking"]
deliverables: ["language metadata policy"]
status: planned
---

# M03-T07 — Detect and record language

## Outcome

Record English, Ukrainian, mixed, unknown, and confidence metadata without using language detection as an authorization decision.
...
2. Record English, Ukrainian, mixed, unknown, and confidence metadata without using language detection as an authorization decision.
...
- Bilingual and short-text fixtures show documented fallback behavior.
...
**Implementation prompt:** Inspect M03-T07 and its prerequisites. Explain the current seam and propose a task-scoped plan for: “Record English, Ukrainian, mixed, unknown, and confidence metadata without using language detection as an authorization decision.” Wait for approval before editing; then report changed files and exact verification results.

**Review prompt:** Review M03-T07 for contract correctness, async/resource safety, security, deterministic tests, and scope. Lead with actionable findings tied to files and behavior; state explicitly if no finding remains.

## Git checkpoint

Commit only after acceptance evidence exists, with intent: `m03-t07: detect and record language`.
